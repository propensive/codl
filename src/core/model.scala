...
object Doc:
  def apply(node: Node*): Doc = Doc(node.to(List), Schema.Free, 0)
...
case class Data(key: Text, children: List[Node] = Nil, layout: Layout = Layout.empty, schema: Schema = Schema.Free)
extends Indexed:
