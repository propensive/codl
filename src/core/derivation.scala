...
trait Codec[T]:
  ...
  protected def readField(nodes: IArray[Node]): Maybe[Text] = nodes match
    case IArray(Node(Data(value, _, _, _, _), _)) => value
    case _                                     => Unset
...
object Codec extends Derivation[Codec]:
  ...
  def join[T](ctx: CaseClass[Codec, T]): Codec[T] = new Codec[T]:
    ...
    def serialize(value: T): IArray[Node] =
      ctx.params.map: p =>
        ...
        Node(Data(p.label.show, p.typeclass.serialize(p.deref(value)), Layout.empty, p.typeclass.schema, index = index))
      .filter(!_.empty)

    def deserialize(value: IArray[Node]): T =
      val dict = value.map { case Node(Data(key, children, _, _, _), _) => key -> children }.to(Map)
  ...
  given list[T](using codec: Codec[T]): Codec[List[T]] = new Codec[List[T]]:
    ...
    def serialize(value: List[T]): IArray[Node] = codec.schema match
      ...
      case Struct(_, _) => IArray.from(value.map { item => Node(Data(t"item", codec.serialize(item), index = Map())) })

    def deserialize(value: IArray[Node]): List[T] = schema match
      case Field(_, _) => value.to(List).map:
        case node@Node(Data(key, _, _, _, _), _) => codec.deserialize(IArray(node))
      case Struct(_, _) => value.to(List).map:
        case Node(Data(_, children, _, _, _), _) => codec.deserialize(children)
